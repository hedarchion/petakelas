  ...
  <header className="lg:hidden flex items-center justify-between p-3 sm:p-4 bg-zinc-900 border-b border-zinc-800 z-50 flex-shrink-0 shadow-lg" role="banner">
    ...
    <div className="flex gap-1.5 sm:gap-2 flex-shrink-0">
      <button 
        ...
        title={viewMode ? "Exit View Mode (V)" : "Enter View Mode (V)"}
...
function AppContent() {
  ...
  const pushToHistory = useCallback((newState: HistoryState) => {
    ...
    setHistory(prev => {
      ...
      const clonedState = typeof structuredClone !== 'undefined' 
        ? structuredClone(newState) 
        : JSON.parse(JSON.stringify(newState));
      const nextHistory = [...newHistory, clonedState];
    ...
    <>
      <div className="flex flex-col lg:flex-row h-screen bg-zinc-950 text-zinc-100 overflow-hidden font-sans" data-device-view="responsive">
        ...
        <aside 
          ...
          aria-hidden={!showMobileSettings && window.innerWidth < 1024}
        >
          <Sidebar 
            classInfo={classInfo}
